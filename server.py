        ...
        while True:
            ...
            if len(datalist) == 0:
                ...
                for tempList in datalist:
                    filename = tempList[0] # Recieve the files name
                    filesize = tempList[1] # Recieve the files size
            ...
            self.checkFiles(clientsocket, tempPath,synchronizationPath)

    # Method compares server files to what was sent
    # We take in the files that do not match or are not known and 
    # add to the synchronized folder to send back 
    def checkFiles(self, clientsocket, tempPath, synchronizationPath):
        print("*********Checking Please Wait***********")
        compare = filecmp.dircmp(tempPath, synchronizationPath)
        tempFileNames = compare.left_list
        syncFileNames = compare.right_list

        for i in tempFileNames:
            if i not in syncFileNames:
                with open(tempPath + "\\" + i, 'rb') as f:
                    data = f.read()
                    with open(synchronizationPath + "\\" + i, 'wb') as f1:
                        f1.write(data)
        self.sendFiles(clientsocket, synchronizationPath)
